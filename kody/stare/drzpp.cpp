...
struct drzewoprzedzialowe //max->min wystarczy zrobic define max min
{
	...
	void dodaj(int p, int k, int war)
	{
		...
		p += pot; k += pot;
		drzewo[p].max += war; drzewo[p].suma += war;//mieszane
		if (p != k)
			drzewo[k].max += war, drzewo[k].suma += war;//mieszane //UWAGA, przecinek!
		while(p>1)
		{
			if (k-p > 1)
			{
				if (p%2 == 0)
					drzewo[p+1].mod += war, drzewo[p+1].max += war, drzewo[p+1].suma += akt*war;//mieszane
				if (k%2==1)
					drzewo[k-1].mod += war, drzewo[k-1].max += war, drzewo[k-1].suma += akt*war;//mieszane
			}
			k/=2; p/=2; akt*=2;
			drzewo[p].max = max(drzewo[2*p].max, drzewo[2*p+1].max) + drzewo[p].mod;//maks
			drzewo[k].max = max(drzewo[2*k].max, drzewo[2*k+1].max) + drzewo[k].mod;//maks
	...
	{
		p += pot; k += pot;
		ll wynp = drzewo[p].max, wynk = drzewo[k].max;
		while(p>1)
		{
			if (k-p > 1)
			{
				if (p%2 == 0)
					wynp = max(wynp, drzewo[p+1].max);
				if (k%2==1)
					wynk = max(wynk, drzewo[k-1].max);
			}
			k/=2; p/=2;
