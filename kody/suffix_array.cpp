...
vector<int> lcp(const string& s, int* sa)
{
    int n=s.size(),k=0;
    vector<int> lcp(n,0);
    vector<int> rank(n,0);
...
int main(){
  cin>>n;
  cin>>s;
  for(int i=0;i<s.size();i++) input[i] = s[i] - 'a' + 1;
  input[n] = 0; input[n+1] = 0; input[n+2] = 0;
  suffixArray(input, SA, n, 26);
}

//krotsza: nie wiem czy dziala LCP
struct SuffixArray {
  const int L;
  string s;
  vector<vector<int> > P;
